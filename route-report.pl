...
my($clear_flights, $clear_delays, $clear_std, $fog_flights, $fog_delays, $fog_std, 
   $rain_flights, $rain_delays, $rain_std, $snow_flights, $snow_delays, $snow_std, 
   $hail_flights, $hail_delays, $hail_std, $thunder_flights, $thunder_delays, $thunder_std,
   $tornado_flights, $tornado_delays, $tornado_std)
 =  (cellValue($row, 'delay:clear_flights'), cellValue($row, 'delay:clear_delays'), cellValue($row, 'delay:clear_std'),
     cellValue($row, 'delay:fog_flights'), cellValue($row, 'delay:fog_delays'), cellValue($row, 'delay:fog_std'),
     cellValue($row, 'delay:rain_flights'), cellValue($row, 'delay:rain_delays'), cellValue($row, 'delay:rain_std'),
     cellValue($row, 'delay:snow_flights'), cellValue($row, 'delay:snow_delays'), cellValue($row, 'delay:snow_std'),
     cellValue($row, 'delay:hail_flights'), cellValue($row, 'delay:hail_delays'), cellValue($row, 'delay:hail_std'),
     cellValue($row, 'delay:thunder_flights'), cellValue($row, 'delay:thunder_delays'), cellValue($row, 'delay:thunder_std'),
     cellValue($row, 'delay:tornado_flights'), cellValue($row, 'delay:tornado_delays'), cellValue($row, 'delay:tornado_std'));
...
print table({-class=>'CSS_Table_Example', -style=>'width:60%;margin:auto;'},
	    Tr([td(['','Clear', 'Fog', 'Rain', 'Snow', 'Hail', 'Thunder', 'Tornado']),
                td(['AVG',average_delay($clear_flights, $clear_delays),
                ...
                td(['STD',sprintf("%.1f", $clear_std),
                    sprintf("%.1f", $fog_std),
                    sprintf("%.1f", $rain_std),
                    sprintf("%.1f", $snow_std),
                    sprintf("%.1f", $hail_std),
                    sprintf("%.1f", $thunder_std),
                    sprintf("%.1f", $tornado_std)])])),
