...
my $records = $hbase->get({
  table => 'weather_delays_by_route_new',
  where => {
...
my($clear_flights, $clear_delays, $clear_sd, $fog_flights, $fog_delays, $fog_sd, $rain_flights, $rain_delays, $rain_sd,
   $snow_flights, $snow_delays, $snow_sd. $hail_flights, $hail_delays, $hail_sd, $thunder_flights, $thunder_delays, $thunder_sd
   $tornado_flights, $tornado_delays, $tornado_sd)
 =  (cellValue($row, 'delay:clear_flights'), cellValue($row, 'delay:clear_delays'), cellValue($row, 'delay:clear_sd'),
     cellValue($row, 'delay:fog_flights'), cellValue($row, 'delay:fog_delays'), cellValue($row, 'delay:fog_sd'),
     cellValue($row, 'delay:rain_flights'), cellValue($row, 'delay:rain_delays'), cellValue($row, 'delay:rain_sd'),
     cellValue($row, 'delay:snow_flights'), cellValue($row, 'delay:snow_delays'), cellValue($row, 'delay:snow_sd'),
     cellValue($row, 'delay:hail_flights'), cellValue($row, 'delay:hail_delays'), cellValue($row, 'delay:hail_sd'),
     cellValue($row, 'delay:thunder_flights'), cellValue($row, 'delay:thunder_delays'), cellValue($row, 'delay:thunder_sd'),
     cellValue($row, 'delay:tornado_flights'), cellValue($row, 'delay:tornado_delays') cellValue($row, 'delay:tornado_sd'));
...
print table({-class=>'CSS_Table_Example', -style=>'width:60%;margin:auto;'},
	    Tr([td(['', 'Clear', 'Fog', 'Rain', 'Snow', 'Hail', 'Thunder', 'Tornado']),
                td(['Average Delay', average_delay($clear_flights, $clear_delays),
		...
		td(['Standard Deviation', $clear_sd, $fog_sd, $rain_sd, $snow_sd,
		    $hail_sd, $thunder_sd, $tornado_sd])])
),
